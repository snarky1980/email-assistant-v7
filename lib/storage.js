...
function atomicWrite(filePath, data) {
  ...
  // fs.renameSync is atomic on POSIX for same filesystem
...
function readTokenStore(file) {
  try { return JSON.parse(fs.readFileSync(file, 'utf8')); } catch (e) { return { tokens: [], updatedAt: null }; }
}
...
function hashToken(token, algo = process.env.TOKEN_HASH_ALGO || 'sha256') {
  return algo + ':' + crypto.createHash(algo).update(token).digest('hex');
}
